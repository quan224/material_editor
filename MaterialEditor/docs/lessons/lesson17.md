...
## 待补（2026-09-12 终裁"渲染不省略"，做本课前先补全教案）

- 延迟渲染 GBuffer 全套（教案现为前向单 pass，需改为 GBuffer pass + 光照 pass）
- 阴影升级为 CSM 级联完整版（现为单 pass 基础阴影）
- Bloom + DOF 后处理（Tonemap 已有）
- 多线程渲染（渲染线程一条，课 16 范围亦可）
- 篇幅装不下时本课后加半课
